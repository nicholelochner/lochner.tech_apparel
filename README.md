...
The browser-side verification footer is packaged as the local npm workspace `@lochner/ipfs-verification-widget` in `packages/ipfs-verification-widget`, so other projects such as `alfmir.ai` can import `createSharedFooterTemplate()` and override the domain, IPNS ID, GitHub manifest URLs, gateway list, and footer credit.

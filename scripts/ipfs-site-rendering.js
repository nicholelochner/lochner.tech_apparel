...
const IPFS_SHARED_FOOTER_TEMPLATE = createSharedFooterTemplate(COPYRIGHT_YEAR);
...
function prepareHtmlForIpfs(html) {
  return html
    ...
    .replace('<!-- SHARED_FOOTER -->', IPFS_SHARED_FOOTER_TEMPLATE)
...
function readIpfsPublishedFile(rootDir, relativePath) {
  const absolutePath = path.join(rootDir, relativePath);
  const data = fs.readFileSync(absolutePath);

  return isHtmlFile(relativePath)
    ? Buffer.from(prepareHtmlForIpfs(data.toString('utf8')), 'utf8')
    : data;
}

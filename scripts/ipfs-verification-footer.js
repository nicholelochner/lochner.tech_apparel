    ...
    <script>
      (function () {
        ...
        async function runVerification() {
          ...
          try {
            const originManifest = await fetchJson(HTTPS_MANIFEST_URL + '?verify=' + Date.now());
            ...
            manifestLinkEl.href = originManifest.source ? originManifest.source + '/' + MANIFEST_PATH : HTTPS_MANIFEST_URL;
            ...
            if (originManifest.gitRevisionDirty || gatewayManifest.gitRevisionDirty) {
              setStatus('warning', 'Uncommitted changes reported; GitHub cannot fully verify this build.');
            } else if (originManifest.contentSha256 === gatewayManifest.contentSha256 && originManifest.gitRevision === gatewayManifest.gitRevision) {
              setStatus('verified', 'Verified: HTTPS and IPFS match the same Git revision.');
            } else if (originManifest.gitRevision !== gatewayManifest.gitRevision) {
              setStatus('warning', 'Mismatch: IPFS points at a different Git revision than HTTPS.');
            } else {
              setStatus('warning', 'Mismatch: IPFS content hash differs from HTTPS.');
            }
          } catch (error) {
